...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ stats, onRestart, onHome }) => {
  const { mode, score, matches, level, round } = stats;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-center">
      <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-8">هذه هي نتائجك في هذه الجلسة.</p>
        ...
        <div className="flex flex-col space-y-4">
          <button 
            onClick={onRestart}
            className="w-full text-lg bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105"
          >
            إعادة اللعب
          </button>
          <button 
            onClick={onHome}
            className="w-full text-lg bg-slate-500 hover:bg-slate-600 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105"
          >
            العودة للرئيسية
          </button>
        </div>
